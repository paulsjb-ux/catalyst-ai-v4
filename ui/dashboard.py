...
def render_dashboard(version: str, scan_results: pd.DataFrame | None = None) -> None:
    ...
    top = frame[frame["signal"].isin(["BUY","WATCH"])].head(8)
    if top.empty:
        empty_state("No qualifying candidates", "The current universe produced no BUY or WATCH signals.", "🛡️")
    else:
        st.dataframe(top[["ticker","signal","score","close","change_20d_pct","rsi_14","trend","reason"]], use_container_width=True, hide_index=True)
