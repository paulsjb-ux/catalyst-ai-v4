...
USER_AGENT = "CatalystAI/8.4 (+market-universe-loader)"
...
def clean_ticker_list(tickers: Iterable[str]) -> list[str]:
    """Clean and de-duplicate tickers while preserving source priority."""
    cleaned: list[str] = []
    seen: set[str] = set()
...
def fetch_sp500_tickers() -> list[str]:
    try:
        ...
        LOGGER.warning("Live S&P 500 universe unavailable; using local fallback: %s", exc)
...
def fetch_nasdaq100_tickers() -> list[str]:
    try:
        ...
        LOGGER.warning("Live Nasdaq-100 universe unavailable; using local fallback: %s", exc)
...
def build_scan_universe(
    ...
    """Build a liquid global universe up to the requested limit.

    Curated global names and the user's watchlist are prioritised before broad
    US index fallbacks, giving larger scans wider geographic coverage.
    """
    tickers: list[str] = []
...
def universe_source_summary(**kwargs) -> dict:
    global_liquid = (
        load_universe_csv(GLOBAL_LIQUID_PATH)
        if kwargs.get("include_global_liquid", True)
        else []
    )
    sp500 = fetch_sp500_tickers() if kwargs.get("include_sp500", True) else []
    nasdaq100 = (
        fetch_nasdaq100_tickers()
        if kwargs.get("include_nasdaq100", True)
        else []
    )
